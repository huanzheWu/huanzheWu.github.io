...
# 0.前言
本文为《深度探索C++对象模型》读后对象内存布局的总结。原文发表于博客园，做了些改进后发表至km，方便日后查阅。文章难免有错漏的地方，如果发现了错误请及时指出以免误导他人。
...
- 理解对象的内存布局之后，可以分析一些问题：
    1. C++封装带来的布局成本是多大？
    2. 由空类组成的继承层次中，每个类对象的大小是多大？

至于其他与内存有关的知识，我假设大家都有一定的了解，如内存对齐，指针相关操作等。
# 3.理解虚函数表
## 3.1.多态与虚表
